...
\maketitle
\begin{abstract}
This document is a user manual describing usage of the JEM software
of the JVET project. It applies to version JEM-7.2, which is based 
on the JCT-VC HM software version 16.6.
\end{abstract}
...
The Joint Exploration Model (JEM) software is the reference software 
of the Joint Video Exploration Team (on Future Video coding) of ITU-T 
VCEG and ISO/IEC MPEG. 
It reflects the explorative work on technologies that may be included
in future video coding standards.

Reference software is being made available to provide a reference
implementation of the technologies being evaluated by JVET.
...
future video coding standard would be.
...
\url{jvet@lists.rwth-aachen.de} (registration required).

\emph{Note, although the software is based on HM, the new technologies
included in JEM are not part of the HEVC video coding standard.}

\subsection*{Bug reporting}
Bugs should be reported on the issue tracker set up at
\url{https://hevc.hhi.fraunhofer.de/trac/jem}

\section{Installation and compilation}
The software may be retrieved from the following SVN server
(mirrored):
\begin{itemize}
\item \url{https://jvet.hhi.fraunhofer.de/svn/svn_HMJEMSoftware/}
\end{itemize}
...
\hline
MS Visual Studio 9     & build/JEM_vc9.sln \\
MS Visual Studio 2010  & build/JEM_vc2010.sln \\
MS Visual Studio 2012  & build/JEM_vc2012.sln \\
MS Visual Studio 2013  & build/JEM_vc2013.sln \\
MS Visual Studio 2015  & build/JEM_vc2015.sln \\
Xcode                  & HM.xcodeproj \\
...
\Option{TrueBitdepthPSNR} &
%\ShortOption{\None} &
\Default{false} &
When 1, compute PSNR using maxval = (1$<<$(bitdepth) - 1) instead of maxval = 255$<<$(bitdepth - 8).
\\

\Option{PreciseLog (-p)} &
%\ShortOption{\None} &
\Default{false} &
Filename to use for producing log file of precise PSNR. If empty, do not produce a file.
\\

\Option{PrintMSSSIM} &
%\ShortOption{\None} &
\Default{false} &
When 1, the MS-SSIM values of each frame and entire sequence will be output.
\\
...
%\ShortOption{\None} &
\Default{true} &
Specifies whether outputing parameter sets before every (intra) random access point
\\
...
%\ShortOption{\None} &
\Default{128} &
Defines the CTU size.
\\

\Option{MinQTLumaISlice} &
%\ShortOption{\None} &
\Default{8} &
Defines the minimum size of luma quadtree leaf node in intra slices.
\\

\Option{MinQTChromaISlice} &
%\ShortOption{\None} &
\Default{4} &
Defines the minimum size of chroma quadtree leaf node in intra slices.
\\

\Option{MinQTNonISlice} &
%\ShortOption{\None} &
\Default{8} &
Defines the minimum size of quadtree leaf node in inter slices.
\\

\Option{MaxBTDepth} &
%\ShortOption{\None} &
\Default{3} &
Defines the maximum depth of binary tree.
\\

\Option{MaxBTDepthISliceL} &
%\ShortOption{\None} &
\Default{3} &
Defines the maximum depth of binary tree for luma component in intra slices.
\\

\Option{MaxBTDepthISliceC} &
%\ShortOption{\None} &
\Default{3} &
Defines the maximum depth of binary tree for chroma components in intra slices.
\\
...
%\ShortOption{\None} &
\Default{true} &
Enables sub-CU level motion vector prediction.
\\

\Option{SubPUTempLog2Size} &
%\ShortOption{\None} &
\Default{2} &
Defines the block size in sub-CU level motion vector prediction.
\\

\Option{OBMC} &
%\ShortOption{\None} &
\Default{true} &
Enables overlapped block motion compensation.
\\

\Option{OBMCBLK} &
%\ShortOption{\None} &
\Default{4} &
Defines the block size in overlapped block motion compensation.
\\

\Option{IMV} &
%\ShortOption{\None} &
\Default{true} &
Enables locally adaptive motion vector resolution. 
\\

\Option{IMVMaxCand} &
%\ShortOption{\None} &
\Default{4} &
Defines the maximum number of candidates searched in locally adaptive motion vector resolution.
\\

\Option{FRUC} &
%\ShortOption{\None} &
\Default{true} &
Enables pattern matched motion vector derivation. 
\\

\Option{FRUCF} &
%\ShortOption{\None} &
\Default{true} &
Enables bilinear filter in pattern matched motion vector derivation.
\\

\Option{FRUCR} &
%\ShortOption{\None} &
\Default{8} &
Defines search range (in pixel) in pattern matched motion vector derivation. 
\\

\Option{FRUCRDepth} &
%\ShortOption{\None} &
\Default{4} &
Defines refine depth in pattern matched motion vector derivation. 
\\

\Option{IlluCompEnable} &
%\ShortOption{\None} &
\Default{true} &
Enables local illumination compensation.
\\

\Option{ALF} &
%\ShortOption{\None} &
\Default{true} &
Enables adaptive loop filter.
\\

\Option{EMT (-emt)} &
%\ShortOption{-emt} &
\Default{3} &
Enables explicit multiple core transform.
\par
\begin{tabular}{cp{0.45\textwidth}}
  0 & Disabled for all blocks. \\
  1 & Enabled for intra blocks. \\
  2 & Enabled for inter blocks. \\
  3 & Enabled for all blocks. \\
\end{tabular}
\\
...
%\ShortOption{-femt} &
\Default{3} &
Enables fast methods for explicit multiple core transform.
\par
\begin{tabular}{cp{0.45\textwidth}}
  0 & Disabled for all blocks. \\
  1 & Enabled for intra blocks. \\
...
%\ShortOption{-klt} &
\Default{0} &
Enables signal dependent transform.
\par
\begin{tabular}{cp{0.45\textwidth}}
  0 & Disabled for all blocks. \\
  1 & Enabled for intra blocks. \\
...
%\ShortOption{\None} &
\Default{true} &
Enables fast methods for large CTU.
\\

\Option{IntraFourTapFilter} &
%\ShortOption{\None} &
\Default{true} &
Enables 4-tap interpolation filter for intra prediction.
\\

\Option{IntraBoundaryFilter} &
%\ShortOption{\None} &
\Default{true} &
Enables additional boundary filter for intra prediction.
\\

\Option{LMChroma} &
%\ShortOption{\None} &
\Default{true} &
Enables cross-component linear model prediction.
\\

\Option{BIO} &
%\ShortOption{\None} &
\Default{true} &
Enables bi-directional optical flow.
\\

\Option{PDPC} &
%\ShortOption{\None} &
\Default{true} &
Enables position dependent intra prediction combination.
\\

\Option{NSST} &
%\ShortOption{\None} &
\Default{true} &
Enables mode dependent non-separable secondary transforms.
\\

\Option{AFFINE} &
%\ShortOption{\None} &
\Default{true} &
Enables affine motion prediction.
\\

\Option{RSAF} &
%\ShortOption{\None} &
\Default{true} &
Enables adaptive reference sample smoothing.
\\

\Option{AClip (-aclip)} &
%\ShortOption{\None} &
\Default{true} &
Enables slice level adaptive clipping.
\\

\Option{DMVR} &
%\ShortOption{\None} &
\Default{true} &
Enable decoder-side motion vector refinement for merge candidates.
\\

\Option{BilateralFilter} &
%\ShortOption{\None} &
\Default{true} &
Enables bilateral filtering.
\\
